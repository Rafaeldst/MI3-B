  ...
  Combattant* choisir_cible(Combattant* lanceur,Combattant* equipe1[], Combattant* equipe2[], Competence* competence,int mode, int difficulte){
    ...
    if(lanceur->equipe==1){
        ...
        if(strcmp(competence->cible, "Ennemi") == 0){
            ...
            choix=meilleur_scan();
            while (choix<1 || choix>TAILLE_EQUIPE || est_invisible(equipe2[choix-1]) || !(vivant(equipe2[choix-1]))){
    ...
    if(lanceur->equipe==2){
        ...
        if(strcmp(competence->cible, "Ennemi") == 0){
            printf("Choisissez une cible :\n");
            choix=meilleur_scan();
            while (choix<1 || choix>TAILLE_EQUIPE || est_invisible(equipe1[choix-1]) || !(vivant(equipe1[choix-1]))){
                printf("Cible incorrect.\n"); 
                choix=meilleur_scan();}
            return equipe1[choix-1];
        }
...
printf("\n");
  ...
  Competence* choisir_attaque(Combattant* combattant) {
    ...
    do {
        ...
        if (choix < 1 || choix > NOMBRE_COMPETENCE) {
